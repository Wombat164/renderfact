...
**OQ8 - ratify the static-asset decision.** Section 5 ends the monolithic UI_HTML pattern at
the workspace boundary (package-data files + allowlisted GET /ui/static/{name}). This changes
how the client ships and deserves a D-number if accepted; the alternative (keep growing
string literals) remains viable only if the vendored D12 libraries are abandoned, which would
reopen D12's preview design. Operator call.
